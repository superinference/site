...
export const metadata: Metadata = {
  title: "Acknowledgements - SuperInference",
  description: "Research and innovation funding sources for SuperInference.",
};
            ...
            {funders.map((f) => (
              <div key={f.logos[0].src} className="flex flex-col items-center gap-4 rounded-xl border border-neutral-200 dark:border-white/10 bg-white dark:bg-neutral-900/60 p-5">
                <div className="flex items-center justify-center gap-4">
                  {f.logos.map((l) => (
                    <Image key={l.src} src={l.src} alt={l.alt} width={l.width} height={95} className="h-14 w-auto" />
                  ))}
                </div>
